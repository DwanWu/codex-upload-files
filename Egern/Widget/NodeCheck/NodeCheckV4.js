/**
 * 节点体检 Widget v4
 * 当前节点：出口 IP、住宅/机房属性、纯净度、中文地区、风险信号、AI 访问状态。
 * IP 情报：IPPure + IPQuery + ip-api.com
 * AI 检测：ChatGPT / Claude / Gemini / Grok
 * Gemini：网页入口 + generativelanguage.googleapis.com 双通道探测。
 */
...
export default async function (ctx) {
  ...
  const probeGemini = async () => {
    const web = await probeService('Gemini', ['https://gemini.google.com/app', 'https://gemini.google.com/']);
    if (web.state === 'limited') return { ...web, via: 'web' };
    if (web.state === 'ok') return { ...web, via: 'web' };

    const api = await probeUrl('https://generativelanguage.googleapis.com/v1beta/models');
    if (api.state === 'ok') return { name: 'Gemini', ...api, via: 'api' };
    if (api.state === 'limited') return { name: 'Gemini', ...api, via: 'api' };
    return { name: 'Gemini', state: 'unknown', status: 0, ms: 0, via: 'none' };
  };
  ...
  try {
    ...
    const purityHero = (scoreSize, labelSize) => col([
      text(String(Math.round(purity)), scoreSize, 'heavy', purityInfo.color, { maxLines: 1 }),
      text(`纯净度 · ${purityInfo.label}`, labelSize, 'bold', purityInfo.color, { maxLines: 1 })
    ], 0, { alignItems: 'end' });
    ...
    if (isSmall) {
      ...
      return {
        ...
        children: [
          row([
            row([icon('shield.checkered', purityInfo.color, 13), text('节点体检', 13, 'heavy', C.main)], 5),
            spacer(),
            purityHero(25, 9)
          ], 6, { alignItems: 'start' }),
          spacer(8),
    ...
    if (isLarge) {
      return {
        ...
        children: [
          row([
            row([icon('shield.checkered', purityInfo.color, 17), text('节点体检', 17, 'heavy', C.main)], 6),
            spacer(),
            purityHero(38, 11)
          ], 8, { alignItems: 'start' }),
          spacer(10),
    ...
    return {
      ...
      children: [
        row([
          row([icon('shield.checkered', purityInfo.color, 15), text('节点体检', 15, 'heavy', C.main)], 6),
          spacer(),
          purityHero(31, 10)
        ], 8, { alignItems: 'start' }),
        spacer(8),
