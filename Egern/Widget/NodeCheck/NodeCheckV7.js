 ...
 * 精简稳定版：纯度 / 属性 / 地区 / 风险
 * 布局参考“广东油价”，不包含 AI 检测。
 * 数据源：IPPure + IPQuery；IP 获取失败时使用 ipify 兜底。
 */
...
export default async function (ctx) {
  ...
  const text = (t, size, weight, color, opts = {}) => ({
    ...
    font: { size, weight }, textColor: color, ...opts
  });
  ...
  try {
    ...
    const cc = String(
      pure.countryCode ||
      iq.location?.country_code ||
      iq.location?.countryCode ||
      ''
    ).toUpperCase();
      ...
      return {
        ...
        children: [
          row([
            ...
            spacer(),
            text(updateTime, 9, 'bold', C.muted)
          ], 0),
          spacer(7),
          col([
      ...
      return {
        ...
        children: [
          row([
            ...
            spacer(),
            text(ip, 11, 'bold', C.muted, { maxLines:1, minScale:0.65 })
          ], 0),
          spacer(14),
          col([
    ...
    const cfg = { radius: 13, padding: [12,6,12,6], labelFz: 11, valueFz: 20, gap: 5 };
    return {
      ...
      children: [
        row([
          ...
          spacer(),
          text(ip, 10, 'bold', C.muted, { maxLines:1, minScale:0.65 })
        ], 0),
        spacer(24),
