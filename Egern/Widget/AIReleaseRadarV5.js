 ...
 * 日期紧跟品牌名显示，避免在 Egern 小/中尺寸被右侧裁切。
 */
...
export default async function (ctx) {
  ...
  let error = '';
  try {
    const resp = await ctx.http.get(`${DATA_URL}?v=5&t=${Date.now()}`, {
  ...
  const compactRow = (r, fontSize = 11) => {
    ...
    return row([
      ...
      text(fmtDate(r.date), fontSize - 1, 'bold', C.blue, { width: 42, maxLines: 1 }),
      ...
      return {
        ...
        children: [
          row([
            ...
            text(fmtDate(r.date, true), 12, 'bold', C.blue),
            spacer(),
